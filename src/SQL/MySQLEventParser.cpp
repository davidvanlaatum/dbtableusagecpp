...
SQL::MySQLEventParser::MySQLEventParser ( SQLParserContext *ctx ) : ctx ( ctx ) {
}

SQL::MySQLEventParser::~MySQLEventParser () {

}

void SQL::MySQLEventParser::parse ( yy::location location, const char *data, size_t len, MySQLEventParserCallback *callback ) {
...
void SQL::MySQLEventParser::parse ( mysql_format_description_event *event, size_t len ) {
  binLogVer = event->ver;
...
char *SQL::MySQLEventParser::extractTableID ( char *data, uint64_t &id, int len ) {
  id = *(uint32_t *) data;
...
void SQL::MySQLEventParser::parseTableMap ( mysql_event *event, size_t len ) {
...
uint8_t SQL::MySQLEventParser::getHeaderLen ( uint8_t type ) {
  return eventHeaderLengths[type];
}

SQL::MySQLTableMapEntry *SQL::MySQLEventParser::getTable ( uint64_t i ) {
  return &tableMap[i];
}
